...
export default function CalendarComponent() {
  ...
  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-3">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full sm:w-auto">
          {/* Legend: on small screens move under title and stack */}
          <div className="flex flex-wrap items-center gap-3">
            {Object.keys(CATEGORY_COLORS).map(cat => (
              <div key={cat} className="flex items-center gap-2 text-sm">
                <span className={`w-3 h-3 rounded-full ${CATEGORY_COLORS[cat]}`}></span>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="inline-flex rounded-md bg-gray-100 p-1">
              <button
                ...
                className={`px-3 py-1 text-sm rounded ${viewMode === 'all' ? 'bg-white shadow' : 'text-gray-600'}`}
              ...
              <button
                ...
                className={`px-3 py-1 text-sm rounded ${viewMode === 'mine' ? 'bg-white shadow' : 'text-gray-600'}`}
          ...
          <div className="flex items-center gap-2 ml-auto">
            <button
              ...
              className="p-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition-colors shadow-md"
            >
              <ChevronLeft className="w-6 h-6" />
            </button>
            <div className="px-6 py-2 font-semibold text-lg text-gray-800">
            ...
            <button
              ...
              className="ml-3 px-3 py-1 bg-gray-100 rounded text-sm hover:bg-gray-200"
            >
              Refresh
            </button>

            <div className="ml-3 text-sm text-gray-500">
              Visible: {Object.keys(eventsByDate).reduce((acc, k) => acc + (eventsByDate[k]?.length || 0), 0)}
            </div>
          </div>
        </div>
      </div>
        ...
        {calendarDays.map((day, idx) => {
          ...
          return (
            <button
              ...
              className={`min-h-[80px] p-2 flex flex-col items-start justify-between border rounded-md transition-colors text-left
